...
export const BlockchainTxModal: React.FC<BlockchainTxModalProps> = ({
  ...
  // When a real txHash arrives, transition to confirming state
  useEffect(() => {
  ...
  useEffect(() => {
    if (isOpen) {
      setPhase('submitting');
      setProgress(0);
      setConfirmedHash(txHash && txHash.startsWith('0x') && txHash.length === 66 ? txHash : null);
      if (txHash && txHash.startsWith('0x') && txHash.length === 66) {
        setPhase('confirming');
      }
    } else {
  ...
  // Progress bar animation — only runs in confirming phase
  useEffect(() => {
